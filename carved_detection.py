    ...
    if pebbleDigitsCrops is not None:
        ...
        for i in range(len(pebbleDigitsCrops)):
            ...
            for f in range(len(fixedImages)):
                ...
                width = int(fixedImages[f].shape[1] * scale_percent / 100)
                height = int(fixedImages[f].shape[0] * scale_percent / 100)
                dim = (width, height)

                downsizedImage = cv2.resize(fixedImages[f], dim, interpolation=cv2.INTER_AREA)
                ...
                if predImg is not None:
                    ...
                    cv2.putText(downsizedImage, 'NONE', (5, 50), cv2.FONT_HERSHEY_SIMPLEX,
                                2, (0, 0, 255), thickness=10)
                    cv2.imwrite(os.path.join(vis_tgt_path, str(i)+str(f)+filename), downsizedImage)
