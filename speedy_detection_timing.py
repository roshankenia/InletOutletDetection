...
class Video():
    def __init__(self, filename):
        ...
        folder = f"./speedy_timing_results/{filename}/"
        if not os.path.isdir(folder):
            os.mkdir(folder)

        # create demo video
        self.processed_video = cv2.VideoWriter(f'./speedy_timing_results/{filename}/processed_video.avi',
                                               cv2.VideoWriter_fourcc(*'mp4v'), self.vidcap.get(cv2.CAP_PROP_FPS), (self.width, self.height))

        self.imgFolder = f"./speedy_timing_results/{filename}/Images/"
    ...
    def processNextFrame(self, frame, frameNumber, inletSavedPebbles=None):
        global currentInVideoTime
        global previousInVideoTime
        global inVideoPebbleTime
        global processingPebbleTime
        og_frame = frame.copy()
        ...
        if pebbleDigitsCrops is not None:
            ...
            inVideoPebbleTime += (currentInVideoTime-previousInVideoTime)

            # note down start of processing time
            ...
            currentPebble, self.activePebbles, self.numOfPebbles = updatePebbleLocation(
                pebbleDigitBoxes[0], self.activePebbles, self.distThreshold, self.numOfPebbles, frameNumber, currentInVideoTime)
            ...
            # note down end of processing time
            processingEndTime = time.time()

            processingPebbleTime += (processingEndTime-processingStartTime)
        # create frame based on current active pebbles
        if inletSavedPebbles is not None:
            frameWithData = addToFrame(
                og_frame, self, frameNumber, currentInVideoTime, inletSavedPebbles)
        else:
            frameWithData = addToFrame(
                og_frame, self, frameNumber, currentInVideoTime)
...
currentInVideoTime = 0
previousInVideoTime = 0

inVideoPebbleTime = 0
processingPebbleTime = 0
...
while inletHasFrames:
    ...
    currentInVideoTime = frameNumber/FPS
    # process inlet frame
    inletVideo.processNextFrame(inletFrame, frameNumber)
    ...
    inletHasFrames, inletFrame = inletVideo.vidcap.read()
    frameNumber += 1
    previousInVideoTime = currentInVideoTime

    print('Amount of Pebble Time:', inVideoPebbleTime)
    print('Amount of Pebble Processing Time:', processingPebbleTime)
